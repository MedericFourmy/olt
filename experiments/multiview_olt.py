...
from olt.config import OBJ_MODEL_DIRS, TrackerConfig, LocalizerConfig, CameraConfig, RegionModalityConfig
from olt.utils import tq_to_SE3, Kres2intrinsics


# TEST_IMG_IDS = [2]   # One view -> high depth uncertainty, depth under estimated
# TEST_IMG_IDS = [2,30]   # Two similar view -> slightly lower uncertainty, depth still under estimated
...
imgs = {img_idx: cv2.cvtColor(img_lst[img_idx], cv2.COLOR_BGR2RGB) for img_idx in TEST_IMG_IDS}
...
T_co_preds, scores = localizer.predict(imgs, Ks, n_coarse=1, n_refiner=3)
...
camera_cfgs = {
    img_idx: CameraConfig(
        color_intrinsics=Kres2intrinsics(K,w,h),
